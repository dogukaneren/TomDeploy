...
if [ "$DEPLOY_TYPE" = "local" ]; then
  ...
  if [ "$AUTH_METHOD" = "key" ]; then
    ...
    ssh -i "$SSH_PRIVATE_KEY" "$REMOTE_USER@$REMOTE_HOST" "sudo systemctl restart tomcat"
    ...
    sshpass -p "$REMOTE_PASSWORD" ssh "$REMOTE_USER@$REMOTE_HOST" "sudo systemctl restart tomcat"
